...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    const currentDate = new Date();
      ...
      // Convertir la fecha actual a la zona horaria de Costa Rica
  const costaRicaOffset = -6; // Costa Rica está en UTC-6
  const currentDateInCR = new Date(currentDate.setHours(currentDate.getHours() + currentDate.getTimezoneOffset() / 60 + costaRicaOffset));

  // Establecer las horas, minutos, segundos y milisegundos a cero para ambas fechas
  birthDate.setHours(0, 0, 0, 0);
  currentDateInCR.setHours(0, 0, 0, 0);
    ...
    if (!birthday) {
      ...
      isValid = false;
    } else if (birthDate.getTime() === currentDateInCR.getTime() || birthDate >= currentDateInCR) {
      setBirthdayError ('La fecha de nacimiento no puede ser una fecha futura');
      isValid = false;
    } else if (age < 13 || (age === 13 && (monthDiff < 0 || (monthDiff === 0 && dayDiff < 0)))) {
      setBirthdayError ('Debes tener al menos 13 años');
      isValid = false;
    }else{
    ...
    if (!phoneRegex.test(phone) || phone.length < 8 || phone.length > 8) { // Cambio en la validación
      setPhoneError('El número de teléfono no puede contener un mínimo de 8 dígitos ni un máximo de 8 dígitos y solo números.');
      isValid = false;
    } else {
      setPhoneError('');
    }
